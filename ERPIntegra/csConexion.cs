...
namespace ERPIntegra
{
    class csConexion
    {
         ...
         public csConexion()
        {
            try
            {
                cn = new SqlConnection("Data Source=CORP_DESARROLLO\\SQLEXPRESS;Initial Catalog=ERPIntegra;Integrated Security=True");
                cn.Open();
            }
            catch (Exception ex)
            {
         ...
         public void BuscaPedido(DataGridView dgv, int estatus)
         {
             try
             {
                   da = new SqlDataAdapter("select  cpFolio as FOLIO ,cpFecha As FECHA,cpIva as IVA_TOTAL,cpSubTot as SUBTOTAL,cpTotal as TOTAL_PEDIDO from tbl_pedidosCab where stsId='" + estatus + "'", cn);
         ...
         public void ExtraePedido(DataGridView dgv, string folio)
         {
             try
             {
                  da = new SqlDataAdapter("select CODIGO,DESCRIPCIÓN,PROVEEDOR,PRECIO,IVA,EXISTENCIA,PZAPEDIR,TOTAL from vw_Pedidos where folio = '" + folio + "'", cn);
         ...
         public string ActualizaDetallepedido(string pc)
         {
             string insertado = "Se guardo corretcamente el pedido";
        ...
        public string ModificaDoctor(int Cedula, string Nombre, string Domicilio, int status, string tipo)
         ...
         public List<lisProveedor> cargacomboProv(ComboBox cmb)
         {
             ...
             using (SqlConnection cnn = new SqlConnection("Data Source=CORP_DESARROLLO\\SQLEXPRESS;Initial Catalog=ERPIntegra;Integrated Security=True"))
             {
                 cnn.Open();
